...
It allows users to upload a CSV file, view the table, compute column statistics (min, max, mean, median, mode, missing count), and plot histograms for numeric columns.
...
Column Stats: Shows min, max, mean, median, mode, and missing count for selected columns.
Histogram Plot: Visualizes numeric column distributions with simple bar charts.
...
Backend (FastAPI)
Endpoint: POST /api/upload → Upload CSV, return dataset ID + schema.
Endpoint: GET /api/dataset/{id}/table → Return table rows.
Endpoint: GET /api/dataset/{id}/column/{col}/stats → Return column statistics.
Endpoint: GET /api/dataset/{id}/column/{col}/hist → Return histogram bins + counts.
